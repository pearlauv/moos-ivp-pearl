...
bool PEARL::HandlePLIMU(string toParse)
{
    vector<string> parts = parseString(toParse, ',');
    if (parts.size() < 5) {
        reportRunWarning("PLIMU message does not contain enough parts: " + toParse);
        return false;
    }
    ...
    try {
        if (!moValStr.empty()) {
            ...
            if (moVal == 0) {
                ...
                reportRunWarning("Manual override command from front seat is not 0 (false) or 1 (true): " + moValStr);
            }
        }
      
        if (!yValStr.empty()) {
            dHeading = stod(yValStr) + m_heading_offset;
            if (dHeading > 360.0) dHeading -= 360.0;
        ...
        reportRunWarning("Conversion error in PLIMU message: " + string(e.what()));
        return false;
    } catch (const out_of_range& e) {
        reportRunWarning("Value out of range in PLIMU message: " + string(e.what()));
        return false;
    }
...
bool PEARL::HandlePLRAW(string toParse)
{
    vector<string> parts = parseString(toParse, ',');
    if (parts.size() < 9) {
        reportRunWarning("PLRAW message does not contain enough parts: " + toParse);
        return false;
    }
    ...
    string mxValStr = parts[7];
    string myValStr = parts[8];
    string mzValStr = parts[9]; // Assuming there’s a 10th part for magnetometer Z value
    
    try {
        double dAccX = stod(axValStr);
        double dAccY = stod(ayValStr);
        double dAccZ = stod(azValStr);
        double dGyroX = stod(gxValStr);
        double dGyroY = stod(gyValStr);
        double dGyroZ = stod(gzValStr);
        double dMagX = stod(mxValStr);
        double dMagY = stod(myValStr);
        double dMagZ = stod(mzValStr);
        
        PublishIMURaw(dAccX, dAccY, dAccZ, dGyroX, dGyroY, dGyroZ, dMagX, dMagY, dMagZ);
    } catch (const invalid_argument& e) {
        reportRunWarning("Conversion error in PLRAW message: " + string(e.what()));
        return false;
    } catch (const out_of_range& e) {
        reportRunWarning("Value out of range in PLRAW message: " + string(e.what()));
        return false;
    }
...
//     m_docked_count = 0;
//     m_auv_count = 1;  // Hardcoded in 1 AUV
//     m_update_str = "points = ";
// }
...
//         if (id_msg.length() == 4) { // Corrected comparison operator
//             new_node_ack = stoi(id_msg.substr(1)); // Changed from erase() to substr()
//         } else {
//             return false;
//         }
