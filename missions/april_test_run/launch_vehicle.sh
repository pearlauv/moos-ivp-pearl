...
for ARGI; do
	...
	echo "  --startpos=X,Y[,HDG] (Default is 0,0)                "
...
#------------------------------------------------------------
#  Normalize sim start position to x,y,heading form.
#------------------------------------------------------------
if [ "${XMODE}" = "SIM" ]; then
    if [[ "${START_POS}" != *,* ]]; then
        echo "launch_vehicle.sh: Bad --startpos format: ${START_POS}"
        exit 1
    elif [[ "${START_POS}" != *,*,* ]]; then
...
if [ "${VERBOSE}" = "yes" ]; then
    ...
    echo "VEHICLE_PORT =  [${VEHICLE_PORT}] "
    echo "SHARE_LISTEN =  [${VEHICLE_LISTEN}]"
    echo "SHORE_IP =      [${SHORE_IP}]     "
    echo "SHORE_LISTEN =  [${SHORE_LISTEN}] "
    echo "----------------------------------"
    echo "VNAME =         [${VNAME}]        "
    echo "REGION =        [${REGION}]       "
    echo "XMODE =         [${XMODE}]        "
