...
- Home / precision target: `x=-200, y=-150`
- Pattern: a 30 m square with corners at `(-200,-150)`, `(-170,-150)`,
  `(-170,-120)`, and `(-200,-120)`

The fixed home point corresponds to approximately
`42.3555241248, -71.1014110416`. For real operation, place the landing target
...
| `LEG 1` | Sends the helm to `(-170,-150)`. | Sends the corresponding GPS waypoint and requests `FLY_WAYPOINT`. |
| `LEG 2` | Sends the helm to `(-170,-120)`. | Sends the corresponding GPS waypoint and requests `FLY_WAYPOINT`. |
| `LEG 3` | Sends the helm to `(-200,-120)`. | Sends the corresponding GPS waypoint and requests `FLY_WAYPOINT`. |
| `HOME` | Sends the helm to fixed mapped home at `(-200,-150)`. | Sends the corresponding fixed GPS waypoint and requests `FLY_WAYPOINT`. |
